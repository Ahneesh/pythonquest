...
let state = {
  ...
  lessonAudioPreferences: JSON.parse(localStorage.getItem("cq_lesson_audio_preferences") || '{"voicePreference":"female","rate":0.9,"voiceURI":null,"codeMode":"explain"}'),
...
async function boot(){
  ...
  const publicShellPromise=Promise.all([
    ...
    if(
      ...
      document.body.dataset.academyDestination!=="signup"
    ){
...
function applySignedOutState(){
  ...
  if(!document.body.dataset.academyDestination){
...
function showAuthenticatedApp(){
  ...
  delete document.body.dataset.academyDestination;
  document.body.classList.remove("auth-pending");
...
const codeQuestSpeechDictionary=[
  [/\bFastAPI\b/gi,"Fast A P I"],
  [/\bREST\b/g,"rest"],
  [/\bAPI\b/g,"A P I"],
  [/\bSQL\b/g,"S Q L"],
  [/\bJVM\b/g,"J V M"],
  ...
  [/\bPyPI\b/g,"Pie P I"],
  [/\bNumPy\b/g,"Num Pie"],
  [/\bPyodide\b/g,"Pie oh dide"],
  [/\bPostgreSQL\b/gi,"post gres Q L"],
  [/\bkubectl\b/gi,"cube control"],
  [/\bOAuth\b/gi,"Oh Auth"],
  [/\bCI\/CD\b/gi,"C I, C D"]
];

function speechNumberWord(value){
  const words=[
...
function normaliseSpeechPunctuation(text){
  let output=String(text||"")
    ...
    .replace(/--/g," decrement ")
    .replace(/&/g," and ")
    .replace(/([a-zA-Z])_([a-zA-Z])/g,"$1 underscore $2")
    .replace(/\b(\d+)\.(\d+)\b/g,(_,major,minor)=>
      `${major} point ${minor.split("").join(" ")}`
    )
  ...
  codeQuestSpeechDictionary.forEach(([pattern,replacement])=>{
...
function speechFriendlySentence(text){
  const cleaned=normaliseSpeechPunctuation(text);
  if(!cleaned)return"";
  return /[.!?]$/.test(cleaned)?cleaned:`${cleaned}.`;
}
function describeCodeForSpeech(code){
  const source=String(code||"").trim();
  if(!source)return"";

  const lines=source.split(/\n+/).map(line=>line.trim()).filter(Boolean);
  const first=lines[0]||"";
  let purpose="a code example";

  if(/\bdef\s+\w+\s*\(/.test(source))purpose="a Python function";
  else if(/\bclass\s+\w+/.test(source))purpose="a class definition";
  else if(/\bSELECT\b/i.test(source))purpose="an S Q L query";
  else if(/\bfetch\s*\(/.test(source))purpose="a web request";
  else if(/\bimport\b/.test(source))purpose="an import example";
  else if(/<\w+[^>]*>/.test(source))purpose="an H T M L example";
  else if(/\b(public|private|protected)\b.*\bclass\b/.test(source))purpose="a Java class";

  const identifierMatch=first.match(
    /\b(?:def|class|function)\s+([A-Za-z_][A-Za-z0-9_]*)/
  );
  const identifier=identifierMatch
    ?` named ${normaliseSpeechPunctuation(identifierMatch[1])}`
    :"";

  return `Code example. This section shows ${purpose}${identifier}. `+
    `It contains ${speechNumberWord(lines.length)} `+
    `${lines.length===1?"line":"lines"} of code. `+
    `Review the code visually, then continue with the explanation.`;
}
function speechBlocksFromElement(element,codeMode="explain"){
  if(!element)return[];

  const clone=element.cloneNode(true);
  clone.querySelectorAll(
    "button,script,style,.textbook-page-number,[aria-hidden='true']"
  ).forEach(node=>node.remove());
  ...
  candidates.forEach((node,index)=>{
    ...
    if(tag==="pre"){
      if(codeMode==="skip"){
        blocks.push("Code example skipped.");
      }else if(codeMode==="literal"){
        blocks.push(
          "Code example. "+
          speechFriendlySentence(raw.replace(/\n+/g,". New line. "))
        );
      }else{
        blocks.push(describeCodeForSpeech(raw));
      }
      return;
    }
    ...
    if(/^h[1-6]$/.test(tag)){
      blocks.push(`Section. ${speechFriendlySentence(raw)}`);
      return;
    }

    if(tag==="li"){
      const parent=node.parentElement;
      const siblings=parent?[...parent.children].filter(
        child=>child.tagName==="LI"
      ):[];
      const position=Math.max(0,siblings.indexOf(node))+1;
      const ordered=parent?.tagName==="OL";
      blocks.push(
        ordered
          ?`Step ${speechNumberWord(position)}. ${speechFriendlySentence(raw)}`
          :speechFriendlySentence(raw)
      );
      return;
    }

    if(tag==="blockquote"){
      blocks.push(`Important note. ${speechFriendlySentence(raw)}`);
      return;
    }

    blocks.push(speechFriendlySentence(raw));
  });

  if(!blocks.length){
    const fallback=speechFriendlySentence(clone.innerText||clone.textContent);
    if(fallback)blocks.push(fallback);
  }

  return blocks.filter(Boolean);
}
function speechScriptFromElement(element,codeMode="explain"){
  return speechBlocksFromElement(element,codeMode).join("   ");
}

function lessonAudioSectionText(shell){
  const page=shell?.querySelector("[data-textbook-page].active");
  if(!page)return"";
  const codeMode=state.lessonAudioPreferences?.codeMode||"explain";
  return speechScriptFromElement(page,codeMode);
}
...
function playLessonAudio(shell){
  ...
  utterance.rate=Math.max(.65,Math.min(1.5,Number(state.lessonAudioPreferences?.rate)||0.9));
...
function lessonAudioControlsHtml(){
  ...
  return`<section class="lesson-audio-reader">
    ...
    <div class="lesson-audio-preferences">
      ...
      <label>Code narration
        <select data-audio-code-mode>
          <option value="explain" ${(state.lessonAudioPreferences?.codeMode||"explain")==="explain"?"selected":""}>Explain code briefly</option>
          <option value="skip" ${state.lessonAudioPreferences?.codeMode==="skip"?"selected":""}>Skip code blocks</option>
          <option value="literal" ${state.lessonAudioPreferences?.codeMode==="literal"?"selected":""}>Read code literally</option>
        </select>
      </label>
      <label>Speed
        <div class="lesson-audio-speed"><input data-audio-rate type="range" min=".65" max="1.5" step=".05" value="${rate}"><strong data-audio-rate-label>${rate.toFixed(2).replace(/0$/,"")}×</strong></div>
      </label>
    </div>
    ...
    <p class="lesson-audio-disclaimer">CodeQuest prepares headings, numbered steps, punctuation and technical terms before speaking. Voice quality still depends on the voices installed in your browser or device.</p>
  </section>`;
}
function bindLessonAudioReader(shell){
  ...
  shell.querySelector("[data-audio-code-mode]")?.addEventListener("change",event=>{
    state.lessonAudioPreferences.codeMode=event.target.value;
    persist();
    if(["speaking","paused"].includes(lessonAudioState.status))playLessonAudio(shell);
  });
  ...
  rateInput?.addEventListener("input",event=>{
    ...
    shell.querySelector("[data-audio-rate-label]").textContent=`${rate.toFixed(2).replace(/0$/,"")}×`;
    persist();
  });
...
const academyCertificateCatalog={
  python:{
    academyId:"python",
    title:"Python Engineering Academy",
    credentialTitle:"Certificate of Completion — Python Engineering",
    description:"Awarded for completing every Python module, lesson step and embedded quiz."
  },
  sql:{
    academyId:"sql",
    title:"SQL & Database Academy",
    credentialTitle:"Certificate of Completion — SQL & Database Engineering",
    description:"Awarded for completing every SQL lesson and passing every module assessment."
  },
  web:{
    academyId:"web",
    title:"Web Development Academy",
    credentialTitle:"Certificate of Completion — Full-Stack Web Foundations",
    description:"Awarded for completing every Web lesson and passing every module assessment."
  },
  java:{
    academyId:"java",
    title:"Java Academy",
    credentialTitle:"Certificate of Completion — Java Engineering",
    description:"Awarded for completing every Java lesson and passing every module assessment."
  }
};
function academyCertificateLessons(academyId){
  if(academyId==="python"){
    const modules=
      (typeof academyModules==="function"&&academyModules("python"))||
      state.academyCatalog?.python?.modules||
      state.academies?.python?.modules||
      [];
    return modules.flatMap(module=>module.lessons||[]);
  }
  if(academyId==="sql")return sqlAllLessons();
  if(academyId==="web")return webAllLessons();
  if(academyId==="java")return javaAllLessons();
  return[];
}
function academyCertificateCurriculumReady(academyId){
  return academyCertificateLessons(academyId).length>0;
}

function academyCertificateCompletedSet(academyId){
  if(academyId==="python")return state.lessons;
  if(academyId==="sql")return state.sqlCompletedLessons;
  if(academyId==="web")return state.webCompletedLessons;
  if(academyId==="java")return state.javaCompletedLessons;
  return new Set();
}
function academyCertificateModules(academyId){
  if(academyId==="python")return academyModules("python");
  if(academyId==="sql")return sqlCourse.modules||[];
  if(academyId==="web")return webCourse.modules||[];
  if(academyId==="java")return javaCourse.modules||[];
  return[];
}
function academyCertificatePythonQuizStatus(){
  const lessons=academyCertificateLessons("python");
  const quizSteps=lessons.flatMap(lesson=>
    (lesson.steps||[])
      .map((step,index)=>({lessonId:lesson.id,index,quiz:Boolean(step.quiz)}))
      .filter(item=>item.quiz)
  );
  const passed=quizSteps.filter(item=>
    new Set(state.steps[item.lessonId]||[]).has(item.index)
  ).length;
  return{passed,total:quizSteps.length,complete:passed===quizSteps.length};
}
function academyCertificateAssessmentStatus(academyId){
  const modules=academyCertificateModules(academyId);
  if(academyId==="python")return academyCertificatePythonQuizStatus();

  const passed=modules.filter(module=>{
    if(academyId==="sql")return sqlExamPassed(module.id);
    if(academyId==="web")return webExamPassed(module.id);
    if(academyId==="java")return javaExamPassed(module.id);
    return false;
  }).length;
  return{passed,total:modules.length,complete:passed===modules.length};
}
function academyCertificateEligibility(academyId){
  const lessons=academyCertificateLessons(academyId);
  const completed=academyCertificateCompletedSet(academyId);
  const completedLessons=lessons.filter(lesson=>completed.has(lesson.id)).length;
  const assessments=academyCertificateAssessmentStatus(academyId);
  const modules=academyCertificateModules(academyId);
  ...
  return{
    ...
    lessons:{
      ...
      complete:lessons.length>0&&completedLessons===lessons.length
    },
    modules:{
...
function academyCompletionCredential(academyId){
  return state.issuedCredentials.find(credential=>
    credential.type==="academy-completion"&&
    credential.academyId===academyId
  )||null;
}
function academyCertificateId(academyId){
  const date=new Date().toISOString().slice(0,10).replace(/-/g,"");
  const random=Math.random().toString(36).slice(2,8).toUpperCase();
  return`CQ-${academyId.toUpperCase()}-${date}-${random}`;
}
function issueAcademyCompletionCertificate(academyId){
  const config=academyCertificateCatalog[academyId];
  const eligibility=academyCertificateEligibility(academyId);
  const existing=academyCompletionCredential(academyId);

  if(existing){
    openAcademyCertificate(existing.id);
    return;
  }
  if(!config||!eligibility.eligible){
    alert("Complete every lesson and pass every required quiz or module assessment first.");
    return;
  }

  const issuedAt=new Date().toISOString();
  const credential={
    id:academyCertificateId(academyId),
    type:"academy-completion",
    academyId,
    certificationId:`academy-${academyId}`,
    title:config.credentialTitle,
    academyTitle:config.title,
    learnerName:state.profile?.name||state.onboardingProfile?.displayName||"CodeQuest Learner",
    issuedAt,
    completedAt:issuedAt,
    issuer:"CodeQuest Academy",
    verificationStatus:"valid",
    evidence:{
      lessons:eligibility.lessons.total,
      modules:eligibility.modules.total,
      assessments:eligibility.assessments.total
    }
  };

  state.issuedCredentials.unshift(credential);
  persist();
  openAcademyCertificate(credential.id);
}
function academyCertificateDocument(credential){
...
<title>${esc(credential.title)}</title>
<style>
  ...
  *{box-sizing:border-box}
  ...
  .certificate{
    ...
    padding:18mm;
    background:
      radial-gradient(circle at top right,#dff5ff 0,transparent 32%),
      radial-gradient(circle at bottom left,#eee5ff 0,transparent 34%),
      #fff;
  ...
  .brand{font-weight:900;letter-spacing:.18em;color:#6d5dfc}
  h1{font-family:Georgia,serif;font-size:42px;margin:14px 0 6px}
  .award{font-size:18px;color:#59627a}
  .name{
  ...
  h2{font-size:28px;margin:10px}
  .summary{max-width:780px;margin:0 auto;line-height:1.6;color:#4d5873}
  .evidence{display:flex;justify-content:center;gap:42px;margin:24px}
  .evidence strong{display:block;font-size:22px}
  .evidence span{font-size:12px;text-transform:uppercase;letter-spacing:.08em}
  .footer{display:flex;justify-content:space-between;text-align:left;font-size:13px}
  .verify{text-align:right}
  .actions{position:fixed;right:20px;top:20px}
  button{padding:12px 18px;border:0;border-radius:10px;background:#16213e;color:#fff;font-weight:700}
...
<main class="certificate">
  <header>
    <div class="brand">CODEQUEST ACADEMY</div>
    <h1>Certificate of Completion</h1>
    <div class="award">This certificate is proudly awarded to</div>
    <div class="name">${esc(credential.learnerName)}</div>
    <div class="award">for successfully completing</div>
    <h2>${esc(credential.academyTitle)}</h2>
    <p class="summary">
      The learner completed the full academy curriculum and passed every
      required quiz or module assessment.
    </p>
    <div class="evidence">
      <div><strong>${evidence.modules||0}</strong><span>Modules</span></div>
      <div><strong>${evidence.lessons||0}</strong><span>Lessons</span></div>
      <div><strong>${evidence.assessments||0}</strong><span>Assessments</span></div>
    </div>
  </header>
  <footer class="footer">
    <div><strong>Issued</strong><br>${issueDate}<br><br><strong>Issuer</strong><br>CodeQuest Academy</div>
    <div class="verify"><strong>Certificate ID</strong><br>${esc(credential.id)}<br><br>Verification status: Valid</div>
  </footer>
</main>
</body>
</html>`;
}
function openAcademyCertificate(credentialId){
  const credential=state.issuedCredentials.find(item=>item.id===credentialId);
  if(!credential)return;
  const popup=window.open("","_blank","noopener,noreferrer");
  if(!popup){
    alert("Allow pop-ups to open the certificate.");
    return;
  }
  popup.document.open();
  popup.document.write(academyCertificateDocument(credential));
  popup.document.close();
}
function academyCompletionCertificateCards(){
  return Object.values(academyCertificateCatalog).map(config=>{
    const eligibility=academyCertificateEligibility(config.academyId);
    const credential=academyCompletionCredential(config.academyId);
    const assessmentLabel=config.academyId==="python"?"embedded quizzes":"module assessments";

    return`<article class="card academy-certificate-card ${credential?"earned":eligibility.eligible?"eligible":""}">
      <div class="academy-certificate-icon">${credential?"✓":"⌁"}</div>
      <div>
        <div class="eyebrow">${credential?"CERTIFICATE ISSUED":eligibility.eligible?"READY TO CLAIM":"ACADEMY COMPLETION"}</div>
        <h2>${config.title}</h2>
        <p>${config.description}</p>
      </div>
      <div class="academy-certificate-requirements">
        <div class="${eligibility.lessons.complete?"passed":""}">
          <span>${eligibility.lessons.complete?"✓":"○"}</span>
          <strong>Lessons</strong>
          <small>${eligibility.lessons.completed}/${eligibility.lessons.total}</small>
        </div>
        <div class="${eligibility.modules.completed===eligibility.modules.total?"passed":""}">
          <span>${eligibility.modules.completed===eligibility.modules.total?"✓":"○"}</span>
          <strong>Modules</strong>
          <small>${eligibility.modules.completed}/${eligibility.modules.total}</small>
        </div>
        <div class="${eligibility.assessments.complete?"passed":""}">
          <span>${eligibility.assessments.complete?"✓":"○"}</span>
          <strong>${assessmentLabel}</strong>
          <small>${eligibility.assessments.passed}/${eligibility.assessments.total}</small>
        </div>
      </div>
      <button
        class="${credential?"secondary-btn":"primary-btn"} full"
        data-academy-certificate="${config.academyId}"
        ${!credential&&!eligibility.eligible?"disabled":""}
      >${credential?"View certificate":eligibility.eligible?"Claim certificate":"Complete academy first"}</button>
      ${credential?`<small class="certificate-id">ID: ${credential.id}</small>`:""}
    </article>`;
  }).join("");
}
...
function renderCertificationHub(){
  el("main").innerHTML=`<section class="certification-hero card"><div><div class="eyebrow">CODEQUEST CREDENTIAL FRAMEWORK</div><h1>Turn learning evidence into structured credentials.</h1><p>Earn an academy completion certificate by finishing every module and passing every required quiz or assessment.</p></div><div class="certification-hero-stat"><strong>${state.issuedCredentials.length}</strong><span>credentials earned</span><small>${state.certificationAttempts.length} exam attempts</small></div></section>
  <div class="section-head"><div><div class="eyebrow">ACADEMY COMPLETION</div><h2>Full academy certificates</h2><p>Issued once the entire academy curriculum and all required knowledge checks are complete.</p></div></div>
  <section class="academy-certificate-grid">${academyCompletionCertificateCards()}</section>
  <div class="section-head"><div><div class="eyebrow">PROFESSIONAL CERTIFICATION</div><h2>Advanced evidence-based credentials</h2></div></div>
  ...
  document.querySelectorAll("[data-academy-certificate]").forEach(button=>{
    button.onclick=()=>issueAcademyCompletionCertificate(button.dataset.academyCertificate);
  });
}
...
function showPublicExperience(view="landing"){
  ...
  if(authRoute){
    openAuthentication(location.hash==="#signin"?"signin":"signup");
    return;
  }
...
function openAuthentication(mode="signin"){
  ...
  // Authentication visibility is the final authority for this transition.
  el("authScreen")?.classList.remove("hidden");
  document.body.classList.add("auth-pending");
}
...
function renderPublicLanding(){
  if(!publicLandingDataReady()){
    el("publicContent").innerHTML=`<section class="landing-loading-shell"><div class="eyebrow">CODEQUEST ACADEMY</div><h1>Preparing your learning experience…</h1></section>`;
    return;
  }
  const slides=[
    {theme:"aurora",badge:"PYTHON + AI",title:"Build production Python systems — not toy scripts.",copy:"Move from typed Python and FastAPI to CI/CD, Kubernetes, LLMs, RAG, MCP and enterprise AI architecture.",skills:["FastAPI","Docker","Kubernetes","RAG","MCP"],outcome:"Enterprise Python & AI Engineer"},
    {theme:"ember",badge:"JAVA AT SCALE",title:"Engineer for throughput, latency and resilience.",copy:"Master concurrency, JVM profiling, GC tuning, Apache Ignite, Kafka or Solace, Oracle, MongoDB and cloud operations.",skills:["Concurrency","JFR","G1/ZGC","Ignite","Kafka"],outcome:"High-Performance Java Engineer"},
    {theme:"electric",badge:"PROOF, NOT PASSIVE VIDEO",title:"Leave with work you can defend in an interview.",copy:"Build systems, write tests, capture performance evidence, document architecture decisions and practise production operations.",skills:["TDD","ADRs","Benchmarks","Runbooks","Capstones"],outcome:"Portfolio-ready evidence"},
    {theme:"prism",badge:"ADAPTIVE PROGRESSION",title:"A structured path that keeps moving with you.",copy:"Mastery checks, revision, projects, professional credentials and daily missions turn ambition into visible progress.",skills:["Mastery","Review","Projects","Credentials","Career tracks"],outcome:"Focused momentum"}
  ];
  el("publicContent").innerHTML=`
  <section class="cq-hero"><div class="cq-hero-orb cq-hero-orb-a"></div><div class="cq-hero-orb cq-hero-orb-b"></div>
    <div class="cq-hero-copy"><div class="eyebrow">FROM FIRST LINE OF CODE TO ENTERPRISE ENGINEERING</div><h1>Build the skills serious engineering teams hire for.</h1><p>CodeQuest combines guided learning, interactive practice, real architecture, portfolio projects and professional pathways across Python, Java, SQL, Web, cloud and AI.</p><div class="cq-hero-actions"><button id="landingStartBtn" class="primary-btn">Start learning free</button><a class="secondary-btn cq-anchor-btn" href="#careerJourneys">Explore pathways</a><a id="landingPricingBtn" class="secondary-btn cq-anchor-btn" href="#pricing">View pricing</a></div><div class="cq-trust-row"><span>✓ Real projects</span><span>✓ Professional pathways</span><span>✓ Mastery progression</span><span>✓ Free starting tier</span></div></div>
    <div class="cq-hero-stage"><div class="cq-code-window"><div class="cq-window-bar"><span></span><span></span><span></span></div><div class="cq-code-line"><b>FastAPI</b><em>production architecture</em></div><div class="cq-code-line"><b>JVM</b><em>profiling and GC tuning</em></div><div class="cq-code-line"><b>RAG + MCP</b><em>governed enterprise AI</em></div><div class="cq-code-line"><b>Kubernetes</b><em>deployment and resilience</em></div></div><div class="cq-floating-badge cq-badge-one">Build</div><div class="cq-floating-badge cq-badge-two">Measure</div><div class="cq-floating-badge cq-badge-three">Deploy</div></div>
  </section>
  <section class="cq-proof-strip"><article><strong>4</strong><span>academies</span></article><article><strong>13</strong><span>professional stages</span></article><article><strong>48</strong><span>enterprise lessons</span></article><article><strong>6</strong><span>portfolio capstones</span></article><article><strong>2</strong><span>advanced certifications</span></article></section>
  <section class="cq-feature-showcase"><div class="cq-section-heading"><div class="eyebrow">ONE PLATFORM. MULTIPLE ENGINEERING IDENTITIES.</div><h2>See what you can become.</h2><p>Explore pathways shaped around real engineering responsibilities.</p></div><div class="cq-carousel-shell" data-feature-carousel><button class="cq-carousel-nav cq-carousel-prev">←</button><div class="cq-carousel-track">${slides.map((x,i)=>`<article class="cq-feature-slide theme-${x.theme} ${i===0?"active":""}" data-carousel-index="${i}"><div class="cq-feature-copy"><span class="cq-feature-badge">${x.badge}</span><h3>${x.title}</h3><p>${x.copy}</p><div class="cq-skill-pills">${x.skills.map(s=>`<span>${s}</span>`).join("")}</div><strong>${x.outcome}</strong></div><div class="cq-feature-art"><div class="cq-art-ring"></div><div class="cq-art-grid"></div><div class="cq-art-card"><span>LEARNING MODE</span><strong>${x.outcome}</strong><small>Architecture • Practice • Evidence</small></div></div></article>`).join("")}</div><button class="cq-carousel-nav cq-carousel-next">→</button><div class="cq-carousel-dots">${slides.map((_,i)=>`<button class="${i===0?"active":""}" data-carousel-dot="${i}"></button>`).join("")}</div></div></section>
  <section class="cq-academy-showcase" id="academies">
      <div class="cq-section-heading">
        <div class="eyebrow">EXPLORE THE ACADEMIES</div>
        <h2>Choose a technology. Build toward a real engineering role.</h2>
        <p>Start with one academy, then combine pathways as your ambitions grow.</p>
      </div>
      <div class="cq-academy-grid">
        <article><span>PY</span><h3>Python Academy</h3><p>Python, FastAPI, CI/CD, Kubernetes, LLMs, RAG and MCP.</p><button class="secondary-btn" data-start-academy="python">Explore Python</button></article>
        <article><span>JV</span><h3>Java Academy</h3><p>Core Java, concurrency, JVM tuning, Ignite, messaging and cloud systems.</p><button class="secondary-btn" data-start-academy="java">Explore Java</button></article>
        <article><span>SQL</span><h3>SQL Academy</h3><p>Data modelling, query optimisation, Oracle and production data engineering.</p><button class="secondary-btn" data-start-academy="sql">Explore SQL</button></article>
        <article><span>WEB</span><h3>Web Academy</h3><p>Modern web foundations, APIs and end-to-end application delivery.</p><button class="secondary-btn" data-start-academy="web">Explore Web</button></article>
      </div>
    </section>

    <section class="cq-how-it-works" id="how">
      <div class="cq-section-heading">
        <div class="eyebrow">HOW IT WORKS</div>
        <h2>Learn in a loop that turns understanding into evidence.</h2>
      </div>
      <div class="cq-how-grid">
        <article><strong>01</strong><h3>Learn</h3><p>Use interactive textbook chapters, examples and guided explanations.</p></article>
        <article><strong>02</strong><h3>Practise</h3><p>Complete coding exercises, labs, mastery checks and independent challenges.</p></article>
        <article><strong>03</strong><h3>Build</h3><p>Create portfolio projects with architecture, tests, deployment and operational evidence.</p></article>
        <article><strong>04</strong><h3>Prove</h3><p>Earn pathway credentials and explain your decisions through verified assessments.</p></article>
      </div>
    </section>

    <section class="cq-role-journeys" id="careerJourneys">
    <div class="cq-section-heading">
      <div class="eyebrow">ROLE-BASED PATHWAYS</div>
      <h2>Choose the engineer you want to become.</h2>
    </div>
    <div class="cq-role-grid">
      <article>
        <span class="cq-role-index">01</span>
        <h3>Enterprise Python &amp; AI Engineer</h3>
        <p>Python → FastAPI → containers → Kubernetes → LLMs → RAG → MCP</p>
        <button class="secondary-btn" data-start-academy="python">Explore Python</button>
      </article>
      <article>
        <span class="cq-role-index">02</span>
        <h3>High-Performance Java Engineer</h3>
        <p>Core Java → concurrency → JVM tuning → Ignite → messaging → cloud</p>
        <button class="secondary-btn" data-start-academy="java">Explore Java</button>
      </article>
      <article>
        <span class="cq-role-index">03</span>
        <h3>Data &amp; SQL Engineer</h3>
        <p>SQL foundations → modelling → optimisation → Oracle → production data systems</p>
        <button class="secondary-btn" data-start-academy="sql">Explore SQL</button>
      </article>
      <article>
        <span class="cq-role-index">04</span>
        <h3>Full-Stack Web Engineer</h3>
        <p>HTML → CSS → JavaScript → APIs → frontend architecture → production delivery</p>
        <button class="secondary-btn" data-start-academy="web">Explore Web</button>
      </article>
    </div>
  </section>
  <section class="cq-value-stack"><div><div class="eyebrow">WHY CODEQUEST FEELS DIFFERENT</div><h2>You do not just watch. You build, test, explain and operate.</h2></div><div class="cq-value-grid"><article><strong>01</strong><h3>Understand</h3><p>Interactive textbook chapters explain concepts and trade-offs.</p></article><article><strong>02</strong><h3>Practise</h3><p>Guided coding, mastery checks and independent challenges.</p></article><article><strong>03</strong><h3>Prove</h3><p>Capstones require tests, evidence and deployment artefacts.</p></article><article><strong>04</strong><h3>Present</h3><p>Credentials and portfolio evidence help you defend your work.</p></article></div></section>
  <section class="cq-faq-section" id="faq">
    <div class="cq-section-heading">
      <div class="eyebrow">QUESTIONS, ANSWERED</div>
      <h2>Everything you need to know before you begin.</h2>
      <p>Explore access, pathways, projects, assessments and subscriptions without leaving the landing page.</p>
    </div>
    <div class="faq-list cq-landing-faq">
      ${landingContent.faq.map((f,i)=>`<article><button data-faq="${i}"><strong>${f.q}</strong><span>+</span></button><p class="hidden" data-faq-answer="${i}">${f.a}</p></article>`).join("")}
    </div>
  </section>
  ${publicPricingCardsHtml()}
  <section class="cq-final-cta"><div><span class="cq-feature-badge">START FREE</span><h2>Your next engineering identity can start with one lesson today.</h2><p>Explore the platform, choose a pathway and build momentum before upgrading.</p></div><button id="landingFinalBtn" class="primary-btn">Create my learning path</button></section>`;
  const member=isAuthenticated();
  el("landingStartBtn").textContent=member?"Go to dashboard":"Start learning free";
  el("landingFinalBtn").textContent=member?"Return to dashboard":"Create my learning path";
  el("landingStartBtn").onclick=publicPrimaryAction;el("landingFinalBtn").onclick=publicPrimaryAction;
  bindPublicPricing(); bindPublicLinks();
  document.querySelectorAll("[data-faq]").forEach(button=>{
    button.onclick=()=>{
      const answer=document.querySelector(
        `[data-faq-answer="${button.dataset.faq}"]`
      );
      if(!answer)return;
  ...
  document.querySelectorAll("[data-start-academy]").forEach(button=>{
    button.onclick=()=>{
      const academyId=button.dataset.startAcademy;
      if(!["python","java","sql","web"].includes(academyId))return;

      setActiveAcademy(academyId);
      syncAcademyIdentity(academyId);
      document.body.dataset.academyDestination="pending";

      if(isAuthenticated()){
        showAuthenticatedApp();
        renderView("academyhome");
        document.body.dataset.academyDestination="academy";
        return;
      }

      // Explore is an explicit signup action. Avoid showAuthScreen(), which
      // performs routing and may restore the public landing during startup.
      applySignedOutState();
  ...
  const shell=document.querySelector("[data-feature-carousel]"); const items=[...shell.querySelectorAll("[data-carousel-index]")]; const dots=[...shell.querySelectorAll("[data-carousel-dot]")]; let active=0;
  const show=i=>{active=(i+items.length)%items.length;items.forEach((x,n)=>x.classList.toggle("active",n===active));dots.forEach((x,n)=>x.classList.toggle("active",n===active))};
  shell.querySelector(".cq-carousel-prev").onclick=()=>show(active-1);shell.querySelector(".cq-carousel-next").onclick=()=>show(active+1);dots.forEach(d=>d.onclick=()=>show(Number(d.dataset.carouselDot)));setInterval(()=>show(active+1),6500);
}
...
function scrollPublicTarget(sectionId){
...
function navigatePublicSection(sectionId){
  if(scrollPublicTarget(sectionId))return;

  renderPublicLanding();
  requestAnimationFrame(()=>{
    requestAnimationFrame(()=>{
      scrollPublicTarget(sectionId);
    });
  });
}
...
function findCodeQuestCredentialById(id){
  const normalised=String(id||"").trim().toUpperCase();
  return state.issuedCredentials.find(credential=>
    String(credential.id||credential.credentialId||"").toUpperCase()===normalised
  )||null;
}
...
function renderAcademyHome(){
  ...
  if(academyId==="sql"){
    renderSqlAcademyHome();
    return;
  }
  if(academyId==="web"){
    renderWebAcademyHome();
    return;
  }
  if(academyId==="java"){
    renderJavaAcademyHome();
  ...
  renderPythonAcademyHome();
}

function renderPythonAcademyHome(){
...
function moduleBelongsToAcademy(module,academyId){
  return Boolean(module&&academyId&&module.academyId===academyId);
}
function academyModules(academyId=state.activeAcademyId||state.activeAcademy){
  return (Array.isArray(course?.modules)?course.modules:[]).filter(module=>moduleBelongsToAcademy(module,academyId));
}
function academyLessonById(id,academyId=state.activeAcademyId||state.activeAcademy){
  return academyModules(academyId).flatMap(module=>Array.isArray(module.lessons)?module.lessons:[]).find(lesson=>lesson.id===id)||null;
}
  ...
  connectivityCheckTimer=setTimeout(async()=>{
    ...
    for(let attempt=0;attempt<2;attempt++){
      try{
        ...
        const response=await fetch(`/index.html?connectivity=${Date.now()}-${attempt}`,{
          method:"HEAD",
          cache:"no-store",
    ...
    // Only show the banner after repeated verified failures.
    showOfflineBanner(failures>=2);
  },1800);
}
...
function lessonSpeechText(){
  ...
  temp.innerHTML=step.content||"";
  const codeMode=state.lessonAudioPreferences?.codeMode||"explain";
  return[
    `Lesson. ${speechFriendlySentence(activeLesson.title)}`,
    `Section. ${speechFriendlySentence(step.title)}`,
    speechScriptFromElement(temp,codeMode)
  ].filter(Boolean).join("  ");
}
function readCurrentLessonAloud(){
  ...
  currentUtterance.rate=Math.max(
    .65,
    Math.min(1.5,Number(state.voiceSettings.rate||state.lessonAudioPreferences?.rate||0.9))
  );
  currentUtterance.pitch=Math.max(
    .8,
    Math.min(1.2,Number(state.voiceSettings.pitch||0.98))
  );
...
async function openLesson(id,step=0,reviewId=null){
  activeLesson=academyLessonById(id);
  if(!activeLesson){
