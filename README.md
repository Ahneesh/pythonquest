...
## v65.0 — Enterprise Python & AI Engineering Pathway

- Adds six advanced Python modules and 24 professional lessons.
- Covers production Python, componentized FastAPI, Docker, CI/CD, Kubernetes, LLM engineering, RAG and MCP.
- Adds five professional pathway stages and credentials.
- Adds three enterprise portfolio projects.
- Adds an Enterprise Python & AI Professional certification.
- Adds Enterprise Python & AI Engineer career and role pathways.
- Extends Python Academy positioning from beginner through enterprise architect.

## v66.0 — High-Performance Java & Distributed Systems Stream

- Adds six advanced Java modules and 24 professional lessons.
- Covers Core Java, multithreading, JVM tuning, distributed systems and Apache Ignite.
- Adds MongoDB, Oracle SQL optimisation, Kafka/Solace, Linux and cloud operations.
- Adds a dedicated TDD and principal architecture capstone.
- Adds four professional stages, three portfolio projects and a new certification.

## v67.0 — Academy Integrity and Landing Refresh

- Explicit academy ownership for every module and lesson.
- Python/Java curriculum isolation.
- New colourful feature carousel and current role pathways.
- Stronger outcome-focused landing page and subscription calls to action.

## v67.1 — Public Pricing Navigation Compatibility Fix

- Restores the visible `View pricing` button in the refreshed landing-page hero.
- Uses the existing public smooth-scroll navigation to reach `#pricing`.
- Preserves desktop and mobile pricing journeys.
- Keeps the v67 academy ownership and landing carousel changes unchanged.

## v67.2 — Landing Usability and Visual Polish

- Adds real Academies and How it works sections for the existing navigation.
- Aligns pricing titles, descriptions, prices, feature lists and CTA buttons.
- Replaces false-positive offline detection with two verified reachability attempts.
- Hides the offline banner while connectivity is being established.
- Improves hero font sizing, line height, tracking and text balance.
- Adds navigation, pricing-alignment and connectivity regression tests.

## v67.3 — Deterministic Landing Navigation QA

- Replaces smooth-scroll navigation with immediate sticky-header-aware scrolling.
- Focuses the destination section after navigation for accessibility and test stability.
- Keeps Academies, How it works, Pricing and FAQ navigation visible on mobile.
- Tests desktop pricing alignment only when cards are displayed horizontally.
- Tests mobile pricing as an ordered, usable stacked layout.
- Uses a small viewport-ratio threshold for long destination sections.

## v67.4 — Native Mobile Fragment Navigation

- Converts Academies, How it works and Pricing header controls into native anchors.
- Converts hero Explore pathways and View pricing controls into native anchors.
- Uses URL fragments as the navigation contract on desktop and mobile.
- Keeps CSS sticky-header offsets through `scroll-margin-top`.
- Updates Playwright to verify destination hashes and section visibility.

## v67.5 — Final Pricing Link Selector Fix

- Updates the last stale Playwright selector from `button` to `link`.
- Keeps the native `#pricing` navigation contract unchanged.
- Preserves all v67.4 landing, academy, authentication and responsive behaviour.

## v67.6 — In-Page FAQ and Valid Connectivity Probe

- Keeps FAQ inside the landing page instead of replacing public content.
- Converts header and footer FAQ controls into native `#faq` links.
- Preserves Academies, How it works and Pricing destinations after FAQ use.
- Moves FAQ accordion binding into the landing-page lifecycle.
- Replaces the nonexistent `/manifest.json` connectivity probe with `/index.html`.
- Counts non-successful connectivity responses as failures.
- Adds FAQ navigation and connectivity-probe regression tests.

## v67.7 — Functional Academy CTAs and Complete Role Grid

- Connects all academy showcase Explore buttons to explicit academy selection.
- Persists selection in both `pq_active_academy` and `cq_active_academy`.
- Updates visible academy identity before opening signup.
- Adds the missing Full-Stack Web Engineer pathway.
- Aligns titles, descriptions and Explore buttons across both landing grids.
- Adds regression coverage for all four academy CTAs and the Web pathway.

## v67.8 — Canonical Academy Selection and Test Isolation

- Routes all landing Explore actions through `setActiveAcademy()`.
- Synchronises visible academy identity with `syncAcademyIdentity()`.
- Assigns `#signup` before opening authentication for signed-out visitors.
- Allows authenticated visitors to enter the chosen academy directly.
- Forces the authenticated Python journey through canonical Python state.
- Prevents stale stored academy values from hiding Python lesson cards.
- Updates CTA tests to accept the correct signed-in or signed-out destination.

## v67.9 — Visible Academy Explore Destination

- Makes every Explore click produce an explicit visible destination.
- Signed-out users are sent through `setAuthMode("signup")` and `showAuthScreen()`.
- Signed-in users render the selected academy home immediately.
- Adds `data-academy-destination` as a deterministic transition signal.
- Preserves the chosen academy while the signup screen is being prepared.
- Updates Playwright to wait for the actual destination rather than sampling state too early.

## v67.10 — Direct Explore Signup Rendering

- Removes `showAuthScreen()` from the signed-out Explore path.
- Applies signed-out cleanup once, then calls `openAuthentication("signup")` directly.
- Makes `openAuthentication()` the final authority for authentication visibility.
- Reasserts the chosen academy after signed-out cleanup.
- Verifies that the signup tab is active as well as the screen being visible.

## v67.11 — Explore Authentication Route Guard

- Prevents `showPublicExperience()` from hiding signup or sign-in routes.
- Protects explicit academy signup destinations from delayed landing renders.
- Extends the public-shell bootstrap guard to respect academy signup intent.
- Adds a regression test that deliberately invokes a late landing render after Explore.

## v67.12 — Speech-Friendly Browser Narration

- Converts headings, paragraphs, numbered lists and bullet points into natural speech blocks.
- Pronounces common CodeQuest technical terms through a dedicated dictionary.
- Reads software versions such as 3.12 as “three point one two”.
- Converts symbols and operators into understandable language.
- Adds Explain, Skip and Literal modes for code narration.
- Uses a calmer 0.9× default rate with a more useful 0.65×–1.5× range.
- Applies the same narration engine to the textbook reader and legacy Listen control.
- Adds regression tests for steps, terminology, versions and code handling.

## v67.13 — Natural Bullet Narration

- Removes the repeated “Next point” phrase from unordered-list narration.
- Reads each bullet directly and naturally.
- Keeps “Step one”, “Step two”, and similar labels for numbered lists.
- Preserves a short pause between spoken items without verbal filler.
- Adds regression coverage for unordered bullets.

## v67.14 — Academy Completion Certificates

- Adds full-academy completion certificates for Python, SQL, Web and Java.
- Requires every academy lesson and module to be complete.
- Requires every embedded Python quiz or every SQL/Web/Java module assessment to be passed.
- Issues each academy certificate only once.
- Stores certificates in the learner’s issued credentials.
- Generates a unique academy-specific certificate ID.
- Provides a printable landscape certificate that can be saved as PDF.
- Displays lesson, module and assessment evidence on the certificate.
- Adds eligibility and certificate-ID regression tests.

## v67.15 — Certificate Curriculum Readiness

- Makes Python certificate lesson discovery resilient to mobile startup order.
- Falls back to available academy catalogue state when the curriculum helper is not ready yet.
- Adds an explicit certificate curriculum-readiness helper.
- Updates the mobile certificate test to wait for curriculum initialisation before asserting counts.

## v67.16 — Academy Identity and Home Routing

- Routes the generic Academy Home to the selected Python, Java, SQL or Web renderer.
- Prevents Web Academy from displaying Python modules and stage readiness.
- Synchronises active academy across state, profile, local storage and document identity.
- Makes academy chooser selection use the canonical academy setter.
- Prevents the header academy badge from remaining stuck on a previous academy.
- Adds regression tests for state synchronisation and academy-specific home routing.
